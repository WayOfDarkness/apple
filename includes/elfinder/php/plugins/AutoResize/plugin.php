...
class elFinderPluginAutoResize extends elFinderPlugin {
	...
	public function onUpLoadMultipleResize(&$thash, &$name, $src, $elfinder, $volume) {
		$opts = $this->getCurrentOpts($volume);
		...
		$sizes = [100, 240, 480, 640, 1024, 2048];
		
		if ($opts['forceEffect'] || $srcImgInfo[0] > $opts['maxWidth'] || $srcImgInfo[1] > $opts['maxHeight']) {
			

			foreach($sizes as $size) {
				$newName = $this->copyFileWithNewSizedName($src, $name, $size, $volume);

				$this->resize($volume, $newName, $srcImgInfo, $size, $size, $opts['quality'], $opts['preserveExif']);

				// resize($volume, $src, $srcImgInfo, $maxWidth, $maxHeight, $jpgQuality, $preserveExif)
			}
			return $this->resize($volume, $src, $srcImgInfo, $opts['maxWidth'], $opts['maxHeight'], $size, $opts['quality'], $opts['preserveExif']);
		}
		
		return false;
	}

	public function onRename($name, $oldPath, $elfinder, $volume) {
		$oldFilename = end(explode('/', $oldPath));
		$srcNoSize = str_replace('/origin/', '/', $oldPath);
		$sizes = [100, 240, 480, 640, 1024, 2048];

		foreach($sizes as $size) {
				$oldSizedPath = $this->addSize($srcNoSize, $size);
				$newSizedPath = str_replace($oldFilename, $name, $oldSizedPath);
		}
	...
	function addSize($name, $size) {
		...
		$len = count($newNames);
		$newNames[$len - 2] .=  '_' . $size;
		return implode('.', $newNames);
	}

	function copyFileWithNewSizedName($src, $name, $size, $volume) {
