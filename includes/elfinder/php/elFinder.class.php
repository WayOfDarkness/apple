...
class elFinder {
	...
	protected function rename($args) {
		...
		// do hook function 'rename.presave'
		if (! empty($this->listeners['rename.presave'])) {
			foreach($this->listeners['rename.presave'] as $handler) {
				$src = $volume->realpath($target);
				call_user_func_array($handler, array(&$name, &$src, $this, $volume));
			}
		}
