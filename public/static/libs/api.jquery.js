...
StoreAPI.getCart = function(callback) {
  jQuery.getJSON('/cart/getInfo', function(cart, textStatus) {
    if ((typeof callback) === 'function') {
      callback(cart);
    } else {
...
StoreAPI.getSale = function(productID, callback) {
  jQuery.getJSON('/api/getSale?product_id=' + productID + '', function(sale, textStatus) {
    if ((typeof callback) === 'function') {
      callback(sale);
    } else {
      StoreAPI.onProduct(sale);
    }
  });
};
