<?php
  ...
  class ShippingFeeRegion extends Illuminate\Database\Eloquent\Model {
    ...
    public function store($data) {
      $shipping_fee = ShippingFeeRegion::where('title',$data['title'])->where('region_id',$data['region_id'])->first();
      if ($shipping_fee) return -1;
      $shipping_fee = new ShippingFeeRegion;
      $shipping_fee->title = $data['title'];
      $shipping_fee->type = $data['type'];
      $shipping_fee->from = $data['from']?$data['from']:0;
      $shipping_fee->to = $data['to']?$data['to']:0;
    ...
    public function update($id, $data) {
      ...
      $shipping_fee->title = $data['title'];
      $shipping_fee->type = $data['type'];
      $shipping_fee->from = $data['from']?:$shipping_fee->from;
      $shipping_fee->to = $data['to']?:$shipping_fee->to;
      $shipping_fee->price = $data['price'];
      $shipping_fee->updated_at = date('Y-m-d H:i:s');
      $shipping_fee->save();
      return $shipping_fee->id;
